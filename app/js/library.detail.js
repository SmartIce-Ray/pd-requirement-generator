...
(function () {
  ...
  function open(it) {
    ...
    (state.config.categories || []).forEach((c) => sel.appendChild(opt(c.name, c.name)));
    // 兼容历史：当前分类已被删除/自定义时仍保留可选
    if (it.category && !(state.config.categories || []).some((c) => c.name === it.category)) sel.appendChild(opt(it.category, it.category));
  ...
  async function saveDetail() {
    ...
    if (!brands.length) { toast("品牌不能为空", true); return; }
    ...
    try { await api.update(it.id, patch); closeModal("#detailModal"); toast("已保存"); L.loadData(); }
    catch (e) { toast(e.message || "保存失败", true); if (e.status === 404) { closeModal("#detailModal"); L.loadData(); } }
  ...
  async function deleteDetail() {
    ...
    try { await api.remove(it.id); closeModal("#detailModal"); toast("已删除"); L.loadData(); }
    catch (e) { toast(e.message || "删除失败", true); if (e.status === 404) { closeModal("#detailModal"); L.loadData(); } }
  }
